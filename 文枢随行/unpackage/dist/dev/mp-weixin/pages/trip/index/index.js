  ...
  methods: {
    loadTripList() {
      ...
      if (savedTrips.length === 0) {
        const latestTrip = common_vendor.index.getStorageSync("latestConfirmedTrip");
        if (latestTrip && typeof latestTrip === "object") {
          savedTrips = [latestTrip];
          common_vendor.index.setStorageSync("myTrips", savedTrips);
        }
      }
      this.tripList = savedTrips.filter((trip) => trip && typeof trip === "object").map((trip) => ({
        ...trip,
        id: trip.id || Date.now(),
        ...
        createTime: trip.createTime || (/* @__PURE__ */ new Date()).toISOString(),
        status: "confirmed",
        statusText: "已确认"
      }));
      common_vendor.index.__f__("log", "at pages/trip/index/index.vue:148", "[trip] loaded list length:", this.tripList.length);
    },
    formatNumber(num) {
    ...
    touchEnd(e) {
      const endX = e.changedTouches && e.changedTouches[0] ? e.changedTouches[0].clientX : this.touchMoveX;
